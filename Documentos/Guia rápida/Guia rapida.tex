...
Dejar una linea vacía equivale a un salto de párrafo. Varias lineas vacías no se toman en cuenta.
...
\begin{center}
\"{}\% Comentario que no aparece en el pdf\"{} = \"{}\"{}\ \ \ \ \ \ \ \ \ \ \ \ \ \
\end{center}
...
\begin{document}
Este es un documento de prueba de \LaTeX en el que 
se muestra la calidad de la tipograf\'ia.
  \begin{align}
    E &= mc^2                              \\
...
\hline
\textbf{Pdf} & \textbf{TeX} & \textbf{Pdf} & \textbf{TeX} \\ \hline
...
Para escribir alguna notación matemática en linea con el texto colocarla entre signos de pesos \$ \$. Ejemplo: 

\begin{center}
\"{}Texto \$E=mc\^{}2\$ Texto.\"{} = \"{}Texto $E=mc^2$ Texto.\"{}
\end{center}
...
Se puede utilizar el paquete \emph{hyperref} (\lstinline$\usepackage{hyperref}$)
...
Para colocar código fuente o texto que utilice muchos caracteres especiales se puede utilizar el paquete \emph{listings}. El paquete soporta los lenguajes de programación más comunes. Para especificar el lenguaje utilizar el comando \lstinline$\lstset{language=lenguajedeseado}$ la documentación del paquete se encuentra en \url{ftp://ftp.tex.ac.uk/tex-archive/macros/latex/contrib/listings/listings.pdf}\\
...
Para código dentro del párrafo se puede utilizar el comando \lstinline!"\lstinline$el codigo$"!. Los signos de pesos \"{}\$\"{} se pueden sustituir por otro que no se encuentre en el código, e.g.~\"{}!\"{}.

\subsection{Listas}

Para crear listas se pueden utilizar los ambientes \emph{enumerate} e \emph{itemize} para crear listas numeradas o con viñetas respectivamente.\\
...
Para dejar más espacio horizontal o verticalmente se pueden utilizar los comandos \lstinline$\vspace{espacio}$ o \lstinline$\hspace{espacio}$ el espacio se puede especificar en cm, mm, pt etc. e.g. 2cm, 15mm, 14pt.

Para hacer que todo el documento o partes tengan interlineado diferente a 1 utilizar el paquete \emph{setspace}. 
...
\textbf{fontenc:} Si solo se utiliza el paquete anterior, al copiar y pegar el texto en los pdfs generados, las letras acentuadas no se copian correctamente. Este paquete soluciona esto y ademas permite escribir algunos caracteres especiales directamente en el archivo TeX.\\
...
\subsection{Ubicación de flotantes}

El paquete \emph{placeins} evita que los floats escritos en una sección aparezcan en la siguiente. Lo que hace es re-definir el comando \textbackslash{}section para que incluya un \textbackslash{}FloatBarrier.\\

\noindent\lstinline$\usepackage[section]{placeins}$

\subsection{Páginas horizontales}

\textbf{pdflscape:} Permite colocar páginas específicas de forma horizontal. Se puede indicar que elementos se quieren rotar (contenido, encabezado y pie de página). Documentación: \url{ftp://ftp.ctan.org/tex-archive/macros/latex/contrib/oberdiek/pdflscape.pdf}.\\
...
Se pueden crear documentos formados por varios archivos TeX. Esto puede ser útil para separar (en capítulos o secciones) documentos muy largos, o para crear muchos documentos que contienen las mismas partes.
...
\newcommand{\titulodocumento}{\LaTeX}
\newcommand{\autor}{Leslie Lamport}
\newcommand{\fecha}{1980}
...
Normalmente LaTeX no reconoce espacios en rutas y nombres de archivos por lo que se recomienda no usar o remplazarlos por guiones bajos (\"{}\_\"{}). Tampoco usar acentos o caracteres especiales.
...
\begin{lstlisting}
\graphicspath{{../../Comun/Imagenes}
              {C:/Users/usuario/Pictures/Imagenes/}}
\end{lstlisting}

\subsection{Configurar Texmaker}

Configurar el editor para que intercambie los tabuladores por un número igual de espacios para todos los colaboradores.

\subsection{Ejemplo}

A continuación se muestra un ejemplo de como se podría ver el archivo TeX de un documento:

\begin{lstlisting}

%% PAQUETES Y ESTILO %%
\input{../Comun/Estilo}
\input{../Comun/Encabezado}
\input{../Comun/Pie}

%% PORTADA %%
\newcommand{\titulodocumento}{Plantilla}
